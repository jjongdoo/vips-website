...
import { adminLogin, getSession, getApprovedAlumni, getAlumniApplications, memberLogin, memberSignUp, getCurrentMember, checkIsAdmin } from '../../lib/supabase';
...
import { TrendingUp, FileText, Users, BarChart3, ChevronRight, ExternalLink, ArrowUpRight, ArrowDownRight, BookOpen, Award, Briefcase, Mail, Globe, Star, Calendar, Target, Zap, LogIn, Shield, RefreshCw, Menu, X, Search, UserPlus, Lock } from "lucide-react";
...
*{box-sizing:border-box}::selection{background:${C.lightBlue}33}button{font-family:inherit}input{font-family:inherit}
@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}
@keyframes slideDown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
.vips-desktop-nav{display:flex !important}.vips-mobile-burger{display:none !important}
.vips-stats-grid{grid-template-columns:repeat(4,1fr) !important}
.vips-two-col{grid-template-columns:1fr 1fr !important}
.vips-hero-btns{flex-direction:row !important}.vips-hero-btns button{justify-content:flex-start !important}
.vips-footer-inner{flex-direction:row !important;text-align:left !important}
.vips-footer-logo{justify-content:flex-start !important}.vips-footer-right{text-align:right !important}
@media(max-width:768px){
.vips-desktop-nav{display:none !important}.vips-mobile-burger{display:flex !important}
...
.vips-hero-btns{flex-direction:column !important}.vips-hero-btns button{justify-content:center !important;width:100% !important}
.vips-footer-inner{flex-direction:column !important;text-align:center !important}
.vips-footer-logo{justify-content:center !important}.vips-footer-right{text-align:center !important}
.vips-main{padding:0 16px 60px !important}.vips-header-inner{padding:0 16px !important}
.vips-hero{padding:32px 24px !important;border-radius:16px !important;margin-top:16px !important}
.vips-hero-title{font-size:36px !important}.vips-hero-title-s{font-size:28px !important}
.vips-hero-sub{font-size:10px !important;letter-spacing:2px !important}.vips-hero-tagline{font-size:14px !important}
.vips-stat-value{font-size:22px !important}.vips-section-title{font-size:20px !important}
.vips-about{padding:24px 20px !important}.vips-card-inner{padding:18px !important}
.vips-research-item{padding:14px 16px !important}.vips-alumni-register{padding:24px 20px !important}
.vips-category-btn{padding:4px 10px !important;font-size:11px !important}
.vips-modal-inner{padding:24px !important}.vips-footer{padding:24px 16px !important}
}
@media(max-width:480px){.vips-hero-title{font-size:30px !important}.vips-hero-title-s{font-size:22px !important}.vips-stat-value{font-size:20px !important}}
`;
...
export default function VIPSHomepage() {
  ...
  const [showLoginModal, setShowLoginModal] = useState(false); // admin login
  ...
  // 회원 시스템
  const [memberUser, setMemberUser] = useState(null); // { name, status, ... }
  ...
  const [signupForm, setSignupForm] = useState({ name: '', email: '', password: '', studentId: '', generation: '' });
    ...
    const f = async () => {
      try {
        ...
        if (session) {
          ...
          if (isAdm) {
            setIsAdmin(true);
            const p = await getAlumniApplications('pending');
            setPendingCount(p?.length || 0);
          }
          const member = await getCurrentMember();
          if (member) setMemberUser(member);
        }
      } catch(e){}
    };
    f();
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      setIsAdmin(true); setShowLoginModal(false); setLoginEmail(''); setLoginPassword('');
      const p = await getAlumniApplications('pending'); setPendingCount(p?.length || 0);
      // 관리자도 member일 수 있음
      const member = await getCurrentMember();
      if (member) setMemberUser(member);
  ...
  const handleMemberLogin = async () => {
    setMemberLoginLoading(true); setMemberLoginError('');
    try {
      const result = await memberLogin(memberLoginEmail, memberLoginPw);
      if (result.member) {
        ...
        // members 테이블에 없으면 관리자일 수 있음
        const isAdm = await checkIsAdmin();
        if (isAdm) { setIsAdmin(true); const p = await getAlumniApplications('pending'); setPendingCount(p?.length || 0); }
      }
      setShowMemberLogin(false); setMemberLoginEmail(''); setMemberLoginPw('');
    } catch(e) { setMemberLoginError('이메일 또는 비밀번호를 확인해주세요.'); }
    finally { setMemberLoginLoading(false); }
  };

  // 회원가입
  const handleSignup = async () => {
    if (!signupForm.name || !signupForm.email || !signupForm.password) { setSignupError('이름, 이메일, 비밀번호는 필수입니다.'); return; }
    if (signupForm.password.length < 6) { setSignupError('비밀번호는 6자 이상이어야 합니다.'); return; }
    setSignupLoading(true); setSignupError('');
    try {
      await memberSignUp(signupForm.email, signupForm.password, signupForm.name, signupForm.studentId, signupForm.generation);
      setSignupDone(true);
    } catch(e) {
      if (e.message?.includes('already registered')) setSignupError('이미 등록된 이메일입니다.');
      else setSignupError('가입 중 오류가 발생했습니다. 다시 시도해주세요.');
    }
  ...
  const handleMemberLogout = async () => {
    ...
    setMemberUser(null); setIsAdmin(false); setPendingCount(0);
  };
  ...
  // Alumni 검색 필터
  const canViewAlumni = isAdmin || (memberUser && memberUser.status === 'approved');
  const filteredAlumni = approvedAlumni.filter(a => {
    if (!alumniSearch.trim()) return true;
    const q = alumniSearch.toLowerCase();
    return (a.name||'').toLowerCase().includes(q) || (a.current_company||'').toLowerCase().includes(q) || (a.generation||'').toLowerCase().includes(q) || (a.job_role||'').toLowerCase().includes(q) || (a.major||'').toLowerCase().includes(q) || (a.student_id||a.graduation_year||'').toString().includes(q);
  });

  const inputBase = { width:'100%',padding:'11px 14px',backgroundColor:C.bg,border:`1px solid ${C.border}`,borderRadius:8,color:C.textPrimary,fontSize:14,outline:'none',boxSizing:'border-box' };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.textPrimary, fontFamily: "'Noto Sans KR','Inter',-apple-system,sans-serif" }}>
          ...
          <nav className="vips-desktop-nav" style={{ alignItems:"center",gap:2 }}>
            ...
            {memberUser ? (
              <div style={{ marginLeft:8,display:'flex',alignItems:'center',gap:6 }}>
                <span style={{ fontSize:12,color:C.textSecondary,fontWeight:500 }}>{memberUser.name}</span>
                <button onClick={handleMemberLogout} style={{ background:'none',border:`1px solid ${C.border}`,color:C.textMuted,cursor:'pointer',padding:'4px 10px',borderRadius:6,fontSize:11 }}>로그아웃</button>
              </div>
            ) : (
              <button onClick={() => setShowMemberLogin(true)} style={{ marginLeft:8,padding:'6px 14px',background:`${C.blue}10`,color:C.blue,border:`1px solid ${C.blue}30`,borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:4 }}><LogIn size={13}/>로그인</button>
            )}

            {/* 관리자 */}
            {!isAdmin ? (
              <button onClick={() => setShowLoginModal(true)} style={{ background:'none',border:'none',color:C.textMuted,cursor:'pointer',padding:'6px',marginLeft:4,opacity:0.3 }} title="관리자"><Shield size={14}/></button>
            ) : (
              <button onClick={() => setShowAdminPanel(true)} style={{ padding:'6px 14px',marginLeft:4,backgroundColor:`${C.blue}15`,color:C.blue,border:`1px solid ${C.blue}33`,borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:5,position:'relative' }}><Shield size={14}/>{lang==='ko'?'관리자':'Admin'}{pendingCount>0&&<span style={{ position:'absolute',top:-6,right:-6,backgroundColor:'#ef4444',color:'#fff',fontSize:10,fontWeight:700,width:18,height:18,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center' }}>{pendingCount}</span>}</button>
            )}
          </nav>
          ...
          <div style={{ borderTop:`1px solid ${C.border}`,marginTop:16,paddingTop:16,display:"flex",flexDirection:"column",gap:8 }}>
            {memberUser ? (
              <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between' }}>
                <span style={{ fontSize:14,color:C.textPrimary,fontWeight:600 }}>{memberUser.name}</span>
                <button onClick={() => { handleMemberLogout(); setMobileMenuOpen(false); }} style={{ padding:'6px 14px',background:'none',border:`1px solid ${C.border}`,borderRadius:8,fontSize:12,color:C.textMuted,cursor:'pointer' }}>로그아웃</button>
              </div>
            ) : (
              <div style={{ display:'flex',gap:8 }}>
                <button onClick={() => { setShowMemberLogin(true); setMobileMenuOpen(false); }} style={{ flex:1,padding:'10px',background:C.blue,color:'#fff',border:'none',borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer' }}>로그인</button>
                <button onClick={() => { setShowSignup(true); setMobileMenuOpen(false); }} style={{ flex:1,padding:'10px',background:'#fff',color:C.blue,border:`1px solid ${C.blue}`,borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer' }}>회원가입</button>
              </div>
            )}
            <div style={{ display:"flex",alignItems:"center",gap:8,marginTop:4 }}>
              ...
              {isAdmin && <button onClick={() => {setShowAdminPanel(true);setMobileMenuOpen(false);}} style={{ padding:'6px 14px',backgroundColor:`${C.blue}15`,color:C.blue,border:`1px solid ${C.blue}33`,borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer' }}><Shield size={14}/></button>}
            </div>
          </div>
        </div>
      )}
      ...
      <main className="vips-main" style={{ maxWidth:1120,margin:"0 auto",padding:"0 24px 80px",opacity:animateIn?1:0,transform:animateIn?"translateY(0)":"translateY(10px)",transition:"all 0.45s cubic-bezier(0.22,1,0.36,1)" }}>
        ...
        {activeTab === "alumni" && (<div style={{ paddingTop:32 }}>
          ...
          {!canViewAlumni ? (
            /* === 로그인 안 된 상태 === */
            <div style={{ ...cardStyle, padding:"48px 32px", textAlign:"center", marginBottom:24 }}>
              <Lock size={32} color={C.textMuted} style={{ marginBottom:12 }} />
              <h3 style={{ color:C.navy, fontSize:18, fontWeight:700, margin:"0 0 8px" }}>
                {lang==='ko'?'Alumni 열람은 회원 전용입니다':'Members Only'}
              </h3>
              <p style={{ color:C.textSecondary, fontSize:14, margin:"0 0 20px", lineHeight:1.6 }}>
                {memberUser && memberUser.status === 'pending' 
                  ? (lang==='ko'?'회원가입 승인 대기 중입니다. 관리자 승인 후 열람 가능합니다.':'Your registration is pending approval.')
                  : memberUser && memberUser.status === 'rejected'
                  ? (lang==='ko'?'회원가입이 거절되었습니다. 관리자에게 문의하세요.':'Your registration was rejected.')
                  : (lang==='ko'?'VIPS 회원으로 로그인하면 Alumni 네트워크를 확인할 수 있습니다.':'Log in as a VIPS member to view the Alumni network.')}
              </p>
              {!memberUser && (
                <div style={{ display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap" }}>
                  <button onClick={() => setShowMemberLogin(true)} style={{ padding:"10px 24px",background:C.heroBg,color:"#fff",border:"none",borderRadius:8,fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6 }}><LogIn size={15}/>로그인</button>
                  <button onClick={() => setShowSignup(true)} style={{ padding:"10px 24px",background:"#fff",color:C.blue,border:`1px solid ${C.blue}`,borderRadius:8,fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6 }}><UserPlus size={15}/>회원가입</button>
                </div>
              )}
            </div>
          ) : (
            ...
            <div>
              ...
              <div style={{ position:'relative', marginBottom:16 }}>
                ...
                <input
                  ...
                  placeholder={lang==='ko'?'이름, 회사, 기수, 학번으로 검색...':'Search by name, company, generation...'}
                ...
                {filteredAlumni.length > 0 ? filteredAlumni.map(a => (
                  <div key={a.id} style={{ ...cardStyle,padding:18 }}>
                    <div style={{ display:"flex",gap:12,alignItems:"flex-start" }}>
                      <div style={{ width:42,height:42,borderRadius:10,flexShrink:0,background:`linear-gradient(135deg,${C.blue}18,${C.lightBlue}10)`,display:"flex",alignItems:"center",justifyContent:"center",border:`1px solid ${C.blue}20`,color:C.blue }}><Users size={18}/></div>
                      <div style={{ flex:1,minWidth:0 }}>
                        <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap" }}>
                          <span style={{ fontWeight:600,fontSize:14,color:C.navy }}>{a.name}</span>
                          {a.generation&&<span style={{ fontSize:10,color:C.blue,background:`${C.blue}0d`,padding:"1px 6px",borderRadius:3,fontWeight:600 }}>{a.generation}</span>}
                          {(a.student_id||a.graduation_year)&&<span style={{ fontSize:10,color:C.textMuted }}>{a.student_id||a.graduation_year}학번</span>}
                        </div>
                        <div style={{ fontSize:13,color:C.textPrimary,marginBottom:1,display:"flex",alignItems:"center",gap:4 }}>
                          <Briefcase size={11} style={{ color:C.textMuted,flexShrink:0 }}/>
                          <span style={{ overflow:"hidden",textOverflow:"ellipsis" }}>{a.job_role}{a.current_company?` @ ${a.current_company}`:''}</span>
                        </div>
                        <div style={{ fontSize:12,color:C.textMuted }}>{a.major}</div>
                        {(a.phone||a.linkedin)&&<div style={{ display:'flex',gap:10,marginTop:6,fontSize:11,color:C.textMuted,flexWrap:'wrap' }}>{a.phone&&<span>📞 {a.phone}</span>}{a.linkedin&&<span>🔗 {a.linkedin}</span>}</div>}
                        {a.message&&<div style={{ fontSize:12,color:C.textSecondary,marginTop:6,fontStyle:'italic' }}>"{a.message}"</div>}
                      </div>
                    </div>
                  </div>
                )) : <div style={{ padding:20,textAlign:'center',color:C.textMuted,fontSize:13,gridColumn:'1/-1' }}>{alumniSearch ? '검색 결과가 없습니다.' : '등록된 Alumni가 없습니다.'}</div>}
              </div>
            </div>
          )}

          {/* Alumni 등록 (누구나 가능) */}
          <div className="vips-alumni-register" style={{ ...cardStyle,marginTop:24,padding:"28px 32px",textAlign:"center",background:`linear-gradient(135deg,${C.blue}08,${C.lightBlue}05)`,border:`1px solid ${C.blue}18` }}>
            ...
            <p style={{ fontSize:13,color:C.textMuted,margin:"0 0 14px" }}>{lang==='ko'?'VIPS 졸업생이신가요? 회원가입 없이 Alumni 등록이 가능합니다.':t.alumni.registerDesc}</p>
            <button onClick={() => setShowAlumniForm(true)} style={{ background:C.heroBg,color:"#fff",border:"none",padding:"10px 24px",borderRadius:8,fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 8px rgba(26,42,94,0.25)" }}>{t.alumni.registerBtn}</button>
          </div>
        </div>)}
      </main>
      ...
      {/* ===== 관리자 로그인 모달 ===== */}
      {showLoginModal && (<div style={{ position:'fixed',top:0,left:0,right:0,bottom:0,backgroundColor:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:10000,padding:20 }} onClick={() => setShowLoginModal(false)}><div className="vips-modal-inner" style={{ backgroundColor:C.card,borderRadius:16,border:`1px solid ${C.border}`,padding:32,width:'100%',maxWidth:400,boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}><div style={{ textAlign:'center',marginBottom:24 }}><div style={{ width:48,height:48,borderRadius:12,margin:'0 auto 12px',background:`${C.blue}12`,display:'flex',alignItems:'center',justifyContent:'center' }}><Shield size={24} color={C.blue}/></div><h3 style={{ color:C.textPrimary,fontSize:18,fontWeight:700,margin:'0 0 4px' }}>관리자 로그인</h3></div>{loginError&&<div style={{ padding:10,backgroundColor:'rgba(220,53,69,0.08)',border:'1px solid rgba(220,53,69,0.2)',borderRadius:8,color:C.red,fontSize:13,marginBottom:16,textAlign:'center' }}>{loginError}</div>}<div style={{ display:'flex',flexDirection:'column',gap:12 }}><input type="email" placeholder="이메일" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} onKeyDown={e => e.key==='Enter'&&handleAdminLogin()} style={inputBase}/><input type="password" placeholder="비밀번호" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} onKeyDown={e => e.key==='Enter'&&handleAdminLogin()} style={inputBase}/><button onClick={handleAdminLogin} disabled={loginLoading} style={{ width:'100%',padding:12,marginTop:4,background:loginLoading?C.border:C.heroBg,color:'#fff',border:'none',borderRadius:8,fontSize:15,fontWeight:700,cursor:loginLoading?'not-allowed':'pointer' }}>{loginLoading?'로그인 중...':'로그인'}</button></div></div></div>)}

      {/* ===== 회원 로그인 모달 ===== */}
      {showMemberLogin && (<div style={{ position:'fixed',top:0,left:0,right:0,bottom:0,backgroundColor:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:10000,padding:20 }} onClick={() => setShowMemberLogin(false)}><div className="vips-modal-inner" style={{ backgroundColor:C.card,borderRadius:16,border:`1px solid ${C.border}`,padding:32,width:'100%',maxWidth:400,boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
        <div style={{ textAlign:'center',marginBottom:24 }}>
          <div style={{ width:48,height:48,borderRadius:12,margin:'0 auto 12px',background:`${C.blue}12`,display:'flex',alignItems:'center',justifyContent:'center' }}><LogIn size={24} color={C.blue}/></div>
          <h3 style={{ color:C.textPrimary,fontSize:18,fontWeight:700,margin:'0 0 4px' }}>로그인</h3>
          <p style={{ color:C.textMuted,fontSize:13,margin:0 }}>VIPS 회원 계정으로 로그인하세요</p>
        </div>
        {memberLoginError&&<div style={{ padding:10,backgroundColor:'rgba(220,53,69,0.08)',border:'1px solid rgba(220,53,69,0.2)',borderRadius:8,color:C.red,fontSize:13,marginBottom:16,textAlign:'center' }}>{memberLoginError}</div>}
        <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
          <input type="email" placeholder="이메일" value={memberLoginEmail} onChange={e => setMemberLoginEmail(e.target.value)} onKeyDown={e => e.key==='Enter'&&handleMemberLogin()} style={inputBase}/>
          <input type="password" placeholder="비밀번호" value={memberLoginPw} onChange={e => setMemberLoginPw(e.target.value)} onKeyDown={e => e.key==='Enter'&&handleMemberLogin()} style={inputBase}/>
          <button onClick={handleMemberLogin} disabled={memberLoginLoading} style={{ width:'100%',padding:12,marginTop:4,background:memberLoginLoading?C.border:C.heroBg,color:'#fff',border:'none',borderRadius:8,fontSize:15,fontWeight:700,cursor:memberLoginLoading?'not-allowed':'pointer' }}>{memberLoginLoading?'로그인 중...':'로그인'}</button>
          <div style={{ textAlign:'center',marginTop:4 }}>
            <button onClick={() => {setShowMemberLogin(false);setShowSignup(true);}} style={{ background:'none',border:'none',color:C.blue,cursor:'pointer',fontSize:13,fontWeight:500 }}>계정이 없으신가요? 회원가입</button>
          </div>
        </div>
      </div></div>)}

      {/* ===== 회원가입 모달 ===== */}
      {showSignup && (<div style={{ position:'fixed',top:0,left:0,right:0,bottom:0,backgroundColor:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:10000,padding:20 }} onClick={() => {setShowSignup(false);setSignupDone(false);setSignupError('');}}><div className="vips-modal-inner" style={{ backgroundColor:C.card,borderRadius:16,border:`1px solid ${C.border}`,padding:32,width:'100%',maxWidth:440,boxShadow:'0 20px 60px rgba(0,0,0,0.15)',maxHeight:'85vh',overflow:'auto' }} onClick={e => e.stopPropagation()}>
        {signupDone ? (
          <div style={{ textAlign:'center',padding:'20px 0' }}>
            <div style={{ width:56,height:56,borderRadius:16,margin:'0 auto 16px',background:'#22c55e18',display:'flex',alignItems:'center',justifyContent:'center' }}><UserPlus size={28} color="#22c55e"/></div>
            <h3 style={{ color:C.textPrimary,fontSize:18,fontWeight:700,margin:'0 0 8px' }}>회원가입 완료!</h3>
            <p style={{ color:C.textSecondary,fontSize:14,lineHeight:1.6,margin:'0 0 20px' }}>관리자 승인 후 Alumni 네트워크를 열람할 수 있습니다.<br/>승인 완료 시 로그인해주세요.</p>
            <button onClick={() => {setShowSignup(false);setSignupDone(false);setSignupForm({name:'',email:'',password:'',studentId:'',generation:''});}} style={{ padding:'10px 32px',backgroundColor:C.blue,color:'#fff',border:'none',borderRadius:8,fontSize:15,fontWeight:600,cursor:'pointer' }}>확인</button>
          </div>
        ) : (<>
          <div style={{ textAlign:'center',marginBottom:24 }}>
            <div style={{ width:48,height:48,borderRadius:12,margin:'0 auto 12px',background:`${C.blue}12`,display:'flex',alignItems:'center',justifyContent:'center' }}><UserPlus size={24} color={C.blue}/></div>
            <h3 style={{ color:C.textPrimary,fontSize:18,fontWeight:700,margin:'0 0 4px' }}>회원가입</h3>
            <p style={{ color:C.textMuted,fontSize:13,margin:0 }}>VIPS 회원으로 가입하면 Alumni 네트워크를 열람할 수 있습니다</p>
          </div>
          {signupError&&<div style={{ padding:10,backgroundColor:'rgba(220,53,69,0.08)',border:'1px solid rgba(220,53,69,0.2)',borderRadius:8,color:C.red,fontSize:13,marginBottom:16,textAlign:'center' }}>{signupError}</div>}
          <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
            <div><label style={{ display:'block',fontSize:12,fontWeight:600,color:C.textSecondary,marginBottom:4 }}>이름 <span style={{ color:C.red }}>*</span></label><input value={signupForm.name} onChange={e => setSignupForm(p=>({...p,name:e.target.value}))} placeholder="홍길동" style={inputBase}/></div>
            <div><label style={{ display:'block',fontSize:12,fontWeight:600,color:C.textSecondary,marginBottom:4 }}>이메일 <span style={{ color:C.red }}>*</span></label><input type="email" value={signupForm.email} onChange={e => setSignupForm(p=>({...p,email:e.target.value}))} placeholder="email@example.com" style={inputBase}/></div>
            <div><label style={{ display:'block',fontSize:12,fontWeight:600,color:C.textSecondary,marginBottom:4 }}>비밀번호 <span style={{ color:C.red }}>*</span> <span style={{ fontWeight:400,color:C.textMuted }}>(6자 이상)</span></label><input type="password" value={signupForm.password} onChange={e => setSignupForm(p=>({...p,password:e.target.value}))} placeholder="••••••" style={inputBase}/></div>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10 }}>
              <div><label style={{ display:'block',fontSize:12,fontWeight:600,color:C.textSecondary,marginBottom:4 }}>학번</label><input value={signupForm.studentId} onChange={e => setSignupForm(p=>({...p,studentId:e.target.value}))} placeholder="예: 20" style={inputBase}/></div>
              <div><label style={{ display:'block',fontSize:12,fontWeight:600,color:C.textSecondary,marginBottom:4 }}>VIPS 기수</label><input value={signupForm.generation} onChange={e => setSignupForm(p=>({...p,generation:e.target.value}))} placeholder="예: 15기" style={inputBase}/></div>
            </div>
            <button onClick={handleSignup} disabled={signupLoading} style={{ width:'100%',padding:12,marginTop:8,background:signupLoading?C.border:C.heroBg,color:'#fff',border:'none',borderRadius:8,fontSize:15,fontWeight:700,cursor:signupLoading?'not-allowed':'pointer' }}>{signupLoading?'가입 중...':'회원가입'}</button>
            <div style={{ textAlign:'center',marginTop:4 }}>
              <button onClick={() => {setShowSignup(false);setShowMemberLogin(true);}} style={{ background:'none',border:'none',color:C.blue,cursor:'pointer',fontSize:13,fontWeight:500 }}>이미 계정이 있으신가요? 로그인</button>
            </div>
          </div>
        </>)}
      </div></div>)}
